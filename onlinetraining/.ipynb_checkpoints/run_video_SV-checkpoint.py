...
dataset = 'ShortVideo-Dataset'
tag = 'ShortVideo-Dataset'
...
image_resize = 224
max_video_no = 153561 # 34321 for 10wu, 91717 for 100wu
...
# 	1	16657 915 9127 3775 9500 10949 5299
'''
...
'''
video_model_load = 'SV' # mvit-base-32x3 slowfast-50 slowfast16x8-101
video_freeze_paras_before = 152# 326 270 576
batch_size_list = [60] # 30 120 120
user_model_load = 'bert4rec'
perceiver = "yes"
...
video_data = f'{data_sample}_dino_video_feature_dict_all.pt'
CVA_depth = 4

if perceiver == "yes":
    exp_name = f"{user_model_load}_{video_model_load}-DINO_{data_sample}_CVA_Depth_{CVA_depth}"
else:
    exp_name = f"{user_model_load}_{video_model_load}-DINO_{data_sample}"
...
epoch = 30
load_ckpt_name = 'None'
# load_ckpt_name = 'epoch-30.pt'

weight_decay = 0.1
drop_rate = 0.1

embedding_dim_list = [384]
lr_list = [5e-4]
text_fine_tune_lr_list = [1e-4]
image_fine_tune_lr_list = [1e-4]
video_fine_tune_lr_list = [1e-3]
index_list = [0]
        ...
        for max_seq_len in max_seq_len_list:
            for index in index_list:
                ...
                run_py = "CUDA_VISIBLE_DEVICES='0,1,2,3' \
                        ...
                        --nproc_per_node 4 --master_port 29700 main_SV.py \
                        --exp_name {} --root_data_dir {} --root_model_dir {} --dataset {} --behaviors {} --text_data {}  --image_data {} --video_data {} --model {} --perceiver {} --CVA_M {}\
                        ...
                        --version {}".format(
                        exp_name, root_data_dir, root_model_dir, dataset, behaviors, text_data, image_data, video_data, user_model_load, perceiver, CVA_depth,
                        mode, item_tower, load_ckpt_name, label_screen, logging_num, save_step,
                        testing_num,weight_decay, drop_rate, batch_size, lr, embedding_dim,
                        image_resize, image_model_load, text_model_load, video_model_load, epoch,
                        text_freeze_paras_before, image_freeze_paras_before, video_freeze_paras_before, max_seq_len, frame_interval, frame_no,
