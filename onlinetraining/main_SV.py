...
from utils.load_data import read_texts, read_behaviors_text, read_items, read_behaviors, read_behaviors_SV, get_doc_input_bert, read_videos, read_videos_from_pairs
...
from utils.dataset import IdDataset, ModalDataset, TextDataset, ImageDataset, VideoDataset, SVFrameDataset, LMDB_Image, LMDB_VIDEO
from utils.metrics import get_item_text_score, get_item_id_score, get_item_image_score, get_item_video_score, get_item_sv_video_score, eval_model, get_fusion_score
...
def print_total_memory_and_exit(local_rank, Log_file):
    ...
    if dist.get_rank() == 0:
        ...
        Log_file.info(f"\n" + "="*50)
        Log_file.info("检测到实验运行中...")
        Log_file.info(f"单卡 (Rank 0) 显存峰值: {peak_mem / (1024**3):.2f} GB")
        Log_file.info(f"所有卡 (Total) 显存峰值总和: {total_mem_gb:.2f} GB")
        Log_file.info(f"="*50 + "\n")
    dist.barrier()
...
def log_memory_snapshot(local_rank, Log_file):
    torch.cuda.synchronize()
...
def train(args, model_dir, Log_file, Log_screen, start_time, local_rank):
    ...
    if 'modal' == args.item_tower or 'video' == args.item_tower:
        if 'video-mae' in args.video_model_load:
            ...
            Log_file.info('load perceiver model...')
            from perceiver_pytorch import PerceiverIO
            video_model = PerceiverIO(
            ...
            Log_file.info('load shortvideo dataset...')
            if args.perceiver == "yes":
                from model.CVA import CVA
                video_model = CVA(
                    dim = args.embedding_dim,          # 每帧 embedding 的维度
                    queries_dim = args.embedding_dim,  # 输出的 embedding 维度
                    logits_dim = args.embedding_dim,   # 输出维度也是 256
                    depth = args.CVA_M,
                    num_latents = 32,
                    latent_dim = 256,
                    cross_heads = 1,
                ...
                video_model = None
        if 'SV' not in args.video_model_load:
    ...
    if 'modal' == args.item_tower or 'image' == args.item_tower or 'video' == args.item_tower or 'id' == args.item_tower:
        ...
        if 'video' == args.item_tower:
            if 'SV' not in args.video_model_load:
                ...
                before_item_id_to_keys, before_item_name_to_id = read_videos_from_pairs(os.path.join(args.root_data_dir, args.dataset, "ShortVideo-Dataset_pairs.csv"))
        else:
    ...
    if 'epoch' in args.load_ckpt_name:
        ...
        if args.mode =='train':
            Log_file.info(f'load {start_epoch} checkpoint...')
            checkpoint = torch.load(ckpt_path, map_location=torch.device('cpu'))
        else:
            Log_file.info('load best checkpoint...')
        ...
        if args.mode =='train':
            ...
            Log_file.info(f'Model loaded from epoch_best.pt')
    ...
    if 'modal' == args.item_tower:
        ...
        train_dataset = SVFrameDataset(u2seq=users_train,
                                    item_num=item_num,
                                    max_seq_len=args.max_seq_len,
    ...
    if 'None' not in args.load_ckpt_name:   # load optimizer status
        ...
        if args.mode == 'train':
            ...
            Log_file.info(f'optimizer loaded from epoch_best.pt')
    ...
    for ep in range(epoch_left):
        ...
        if not need_break and (now_epoch-1) % 1 == 0 and now_epoch > 1:
            ...
            if args.mode == 'train':
                max_eval_value, max_epoch, early_stop_epoch, early_stop_count, need_break = eval(now_epoch, max_epoch, early_stop_epoch, 
                                                                                                 max_eval_value, early_stop_count, model, 
                                                                                                 users_history_for_valid, users_valid, 64, 
                                                                                                 item_num, args.mode, is_early_stop, 
                                                                                                 local_rank, args, pop_prob_list, Log_file, 
                                                                                                 item_content, item_id_to_keys, 10)
        ...
        if args.mode == 'test':
            max_eval_value, max_epoch, early_stop_epoch, early_stop_count, need_break = eval(now_epoch, max_epoch, early_stop_epoch, 
                                                                                             ...
                                                                                             local_rank, args, pop_prob_list, Log_file, 
                                                                                             item_content, item_id_to_keys, 10)
            max_eval_value_20, max_epoch_20, early_stop_epoch_20, early_stop_count_20, need_break_20 = eval(now_epoch, max_epoch, 
                                                                                                            early_stop_epoch, max_eval_value, 
                                                                                                            early_stop_count, model, 
                                                                                                            users_history_for_valid, users_valid, 64, 
                                                                                                            item_num, args.mode, is_early_stop, 
                                                                                                            local_rank, args, pop_prob_list, Log_file, 
                                                                                                            item_content, item_id_to_keys, 20)
            return
        ...
        for data in train_dl:
            ...
            # print_total_memory_and_exit(local_rank, Log_file)
            if ep == 0 and batch_index == 5: 
                log_memory_snapshot(local_rank, Log_file)
...
def eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count, model, user_history, users_eval, batch_size, item_num,\
    ...
    if 'text' == args.item_tower:
        ...
        if 'SV' not in args.video_model_load:
            ...
            item_scoring = get_item_sv_video_score(model, item_num, item_id_to_keys, batch_size, args, local_rank)
