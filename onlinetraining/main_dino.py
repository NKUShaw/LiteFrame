...
from transformers import VideoMAEModel, AutoImageProcessor, VideoMAEConfig
...
from utils.logging_utils import para_and_log, report_time_train, report_time_eval, save_model, save_best_model, setuplogger, get_time
from utils.dataset import IdDataset, ModalDataset, TextDataset, ImageDataset, VideoDataset, DINOFrameDataset, LMDB_Image, LMDB_VIDEO
from utils.metrics import get_item_text_score, get_item_id_score, get_item_image_score, get_item_video_score, get_item_dino_video_score, eval_model, get_fusion_score
...
def print_total_memory_and_exit(local_rank):
    torch.cuda.synchronize() 
    
    peak_mem = torch.cuda.max_memory_allocated(local_rank)
    
    mem_tensor = torch.tensor([peak_mem], dtype=torch.float64, device=local_rank)
    
    dist.all_reduce(mem_tensor, op=dist.ReduceOp.SUM)
    
    if dist.get_rank() == 0:
        total_mem_gb = mem_tensor.item() / (1024 ** 3)
        print("\n" + "="*50)
        print(f"检测到实验运行中...")
        print(f"单卡 (Rank 0) 显存峰值: {peak_mem / (1024**3):.2f} GB")
        print(f"所有卡 (Total) 显存峰值总和: {total_mem_gb:.2f} GB")
        print("="*50 + "\n")
...
def log_memory_snapshot(local_rank):
    torch.cuda.synchronize()
    ...
    if dist.get_rank() == 0:
        ...
        print("\n" + "-"*50)
        print(f"【显存监控】")
        print(f"单卡 (Rank 0) 峰值: {peak_mem / (1024**3):.2f} GB")
        print(f"所有卡 (Total) 总和: {total_mem_gb:.2f} GB")
        print("-"*50 + "\n")
...
def train(args, model_dir, Log_file, Log_screen, start_time, local_rank):
    ...
    if 'modal' == args.item_tower or 'video' == args.item_tower:
        if 'video-mae' in args.video_model_load:
            ...
            configuration = VideoMAEConfig.from_pretrained("MCG-NJU/videomae-base", num_frames=args.frame_no)
            video_model = VideoMAEModel.from_pretrained("MCG-NJU/videomae-base", config=configuration)
            ...
            # video_model.load_state_dict(torch.load('./pretrained/MVIT_B_32x3_f294077834.pyth'), strict=False)
            video_model = torch.hub.load("facebookresearch/pytorchvideo", "mvit_base_32x3", pretrained=True)
            ...
            Log_file.info('load dinov3 model...')
            # video_model = torch.hub.load(
            #                         "../../../../../dinov3", 
            #                         'dinov3_vits16', 
            #                         source="local", 
            #                         weights="../../../../../dinov3/weights/dinov3_vits16_pretrain_lvd1689m.pth"
            #                        )
            # from perceiver_pytorch import PerceiverIO
            # from model.gated_perceiver_io import PerceiverIO
            # from model.ablation import CVA
            # video_model = CVA(
            #     dim = args.embedding_dim,          # 每帧 embedding 的维度
            #     queries_dim = args.embedding_dim,  # 输出的 embedding 维度
            #     logits_dim = args.embedding_dim,   # 输出维度也是 384
            #     depth = args.CVA_M,
            #     num_latents = 32,
            #     latent_dim = 256,
            #     cross_heads = 1,
            ...
            # )
            # video_model.query = nn.Parameter(torch.randn(1, 1, args.embedding_dim))
            # video_model.query = None
            from model.CVA import CVA
            video_model = CVA(
                dim = args.embedding_dim,          # 每帧 embedding 的维度
                queries_dim = args.embedding_dim,  # 输出的 embedding 维度
                logits_dim = args.embedding_dim,   # 输出维度也是 384
            ...
            video_model.query = None
            # from model.ablation import MLP
            # video_model = MLP(dim = args.embedding_dim,
            #                   mult = 4)
            # video_model.query = None
            # from model.ablation import PoolingIdentity
            # video_model = PoolingIdentity()
            # video_model.query = None
        if 'dinov3' not in args.video_model_load:
    ...
    if 'modal' == args.item_tower or 'image' == args.item_tower or 'video' == args.item_tower or 'id' == args.item_tower:
        ...
        # import json
        # json.dump(item_id_to_keys, open("item_id_to_keys.json", "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    ...
    if 'epoch' in args.load_ckpt_name:
        ...
        best_path = os.path.abspath(os.path.join(model_dir, "epoch_best.pt"))

        if "best" not in args.load_ckpt_name:
    ...
    if 'modal' == args.item_tower:
        ...
        train_dataset = DINOFrameDataset(u2seq=users_train,
                                    item_num=item_num,
                                    max_seq_len=args.max_seq_len,
                                    embedding_path=os.path.join(args.root_data_dir, args.dataset, args.video_data),
                                    item_id_to_keys=item_id_to_keys,
    ...
    for ep in range(epoch_left):
        ...
        if not need_break and (now_epoch-1) % 1 == 0 and now_epoch > 1:
            ...
            max_eval_value, max_epoch, early_stop_epoch, early_stop_count, need_break = \
                eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count, model, users_history_for_valid, \
                    users_valid, 64, item_num, args.mode, is_early_stop, local_rank, args, pop_prob_list, Log_file, item_content, item_id_to_keys, 10)

            if max_eval_value > old_best:
                if args.mode == 'train':
                    save_best_model(now_epoch, model, model_dir, optimizer, torch.get_rng_state(), torch.cuda.get_rng_state(), Log_file)
        if args.mode == 'test':
            max_eval_value, max_epoch, early_stop_epoch, early_stop_count, need_break = eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count, model, users_history_for_valid, users_valid, 64, item_num, args.mode, is_early_stop, local_rank, args, pop_prob_list, Log_file, item_content, item_id_to_keys, 10)
            max_eval_value_20, max_epoch_20, early_stop_epoch_20, early_stop_count_20, need_break_20 = eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count, model, users_history_for_valid, users_valid, 64, item_num, args.mode, is_early_stop, local_rank, args, pop_prob_list, Log_file, item_content, item_id_to_keys, 20)
            return
        ...
        for data in train_dl:
            if 'modal' == args.item_tower:
                ...
                sample_items_id, sample_items_video, log_mask = data # sample_items_id 前10个历史视频+当前预测视频, sample_items_video 视频数据, log_mask 前10个历史视频的padding部分
            ...
            # print_total_memory_and_exit(local_rank)
            if ep == 0 and batch_index == 5: 
                log_memory_snapshot(local_rank)
...
def eval(now_epoch, max_epoch, early_stop_epoch, max_eval_value, early_stop_count, model, user_history, users_eval, batch_size, item_num,\
     mode, is_early_stop, local_rank, args, pop_prob_list, Log_file, item_content=None, item_id_to_keys=None, topK=10):
    ...
    if 'text' == args.item_tower:
        ...
        if 'dinov3' not in args.video_model_load:
            ...
            item_scoring = get_item_dino_video_score(model, item_num, item_id_to_keys, batch_size, args, local_rank)
    ...
    valid_Hit10, nDCG10 = eval_model(model, user_history, users_eval, item_scoring, batch_size, \
        args, item_num, Log_file, mode, pop_prob_list, local_rank, now_epoch, topK = topK)
...
def main():
    ...
    seconds_all = hour * 3600 + minute * 60 + seconds
    Log_file.info('#### (time) all: hours {} minutes {} seconds {} ####'.format(hour, minute, seconds))
    Log_file.info('#### (time) all: seconds {} ####'.format(seconds_all))
