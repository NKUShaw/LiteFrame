...
import os
import math
from .dataset import  EvalDataset, SequentialDistributedSampler, LmdbEvalDataset, IdEvalDataset, ItemsDataset, DINOFrameEvalDataset, SVFrameEvalDataset
...
def get_item_sv_video_score(model, item_num, item_id_to_keys, test_batch_size, args, local_rank):
    model.eval()
    embedding_dict = torch.load(os.path.join(args.root_data_dir, args.dataset, args.video_data))

    item_dataset = SVFrameEvalDataset(item_ids=np.arange(item_num + 1), item_id_to_keys=item_id_to_keys,
                                        embedding_dict=embedding_dict, frame_no=args.frame_no,)
    item_dataloader = DataLoader(item_dataset, batch_size=test_batch_size,
                                 num_workers=args.num_workers, pin_memory=True)
    item_scoring = []
    if args.perceiver == 'yes':         
        video_encoder = model.module.video_encoder
        video_encoder = video_encoder.to(local_rank)
        video_encoder.eval()
    
        with torch.no_grad():
            for input_ids in item_dataloader:
                input_ids = input_ids.to(local_rank)
                B, F, D = input_ids.shape
                if video_encoder.query:
                    queries = video_encoder.query.expand(B, 1, D).to(local_rank)
                else:
                    queries = video_encoder.query
                input_ids = video_encoder(input_ids.reshape(B, F, D), queries=queries)
                input_ids = input_ids.squeeze(1) 
                
                item_scoring.extend(input_ids)
        return torch.stack(tensors=item_scoring, dim=0).to(torch.device('cpu')).detach()
    else:
        with torch.no_grad():
            for input_ids in item_dataloader:
                # print(input_ids.shape) BatchSize 8 256
                video_emb = input_ids.mean(dim=1)
                item_scoring.append(video_emb.cpu())
                
        return torch.cat(item_scoring, dim=0).to(torch.device('cpu')).detach()
    
def get_item_dino_video_score(model, item_num, item_id_to_keys, test_batch_size, args, local_rank):
    model.eval()
    embedding_dict = torch.load(os.path.join(args.root_data_dir, args.dataset, args.video_data))
    item_dataset = DINOFrameEvalDataset(item_ids=np.arange(item_num + 1), item_id_to_keys=item_id_to_keys,
                                        embedding_dict=embedding_dict, frame_no=args.frame_no,)
    item_dataloader = DataLoader(item_dataset, batch_size=test_batch_size,
                                 num_workers=args.num_workers, pin_memory=True)
    item_scoring = []
...
def eval_model(model, user_history, eval_seq, item_scoring, test_batch_size, args, item_num, Log_file, v_or_t, pop_prob_list, local_rank, epoch, topK = 10):
    ...
    with torch.no_grad():
        ...
        for data in eval_dl:
            ...
            if args.model == 'sasrec' or args.model == 'bert4rec':
            ...
            for user_id, label, score in zip(user_ids, labels, scores):
                ...
                gt_item_list.append(eval_seq[user_id][-1])
        ...
        # np.save(f'./results/rank_list_{topK}.npy', np.array(rank_list))
        # np.save(f'./results/user_list_{topK}.npy', np.array(user_list))
        # np.save(f'./results/item_list_{topK}.npy', np.array(item_list))
        # np.save(f'./results/gt_list_{topK}.npy', np.array(gt_item_list))
