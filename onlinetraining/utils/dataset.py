...
class VideoDataset(Dataset):
    def __init__(self, u2seq, item_num, max_seq_len, item_id_to_keys, db_path, frame_no):
        self.u2seq = u2seq # 每个用户访问的视频序列
        self.item_num = item_num # 19099
        self.max_seq_len = max_seq_len + 1 # 训练样本长度上限 + target（预测下一个 item）
        self.item_id_to_keys = item_id_to_keys
    ...
    def __getitem__(self, user_id):
        ...
        seq_Len = len(seq)
        tokens_Len = len(seq) - 1
        mask_len_head = self.max_seq_len - seq_Len # 11 - seq_Len
        log_mask = [0] * mask_len_head + [1] * tokens_Len # 掩码标出哪些位置是有效的历史视频（1），哪些是无效的padding（0）。

        sample_items = np.zeros((self.max_seq_len, self.frame_no, 3, 224, 224)) 
        sample_id_items = [0] * mask_len_head + seq # 不够的部分填充在最左边
        ...
        sample_id_items = torch.LongTensor(sample_id_items)
        sample_items = torch.FloatTensor(sample_items)
        return sample_id_items, sample_items, torch.FloatTensor(log_mask) # Padding访问序列, 视频, mask
        
class SVFrameDataset(Dataset):
    def __init__(self, u2seq, item_num, max_seq_len, item_id_to_keys, embedding_path, frame_no):
        self.u2seq = u2seq # 每个用户访问的视频序列
        self.item_num = item_num # 19099
        self.max_seq_len = max_seq_len + 1 # 训练样本长度上限 + target（预测下一个 item）
        self.item_id_to_keys = item_id_to_keys
        
        self.embedding_dict = torch.load(embedding_path)
        self.frame_no = frame_no
        example_key = next(iter(self.embedding_dict))
        self.embed_dim = self.embedding_dict[example_key].shape[1]
    ...
    def __getitem__(self, user_id):
        ...
        sample_items = np.zeros((self.max_seq_len, self.frame_no, self.embed_dim), dtype=np.float32)
        sample_id_items = [0] * mask_len_head + seq # 不够的部分填充在最左边

        def get_real_id(item_id):
            return self.item_id_to_keys[item_id]

        for i in range(tokens_Len):
            item_id = seq[i]
            real_id = get_real_id(item_id)

            feat = self.embedding_dict[real_id]         # (8,256)       
            sample_items[mask_len_head + i] = feat[:self.frame_no] 

        last_item_id = seq[-1]
        real_id = get_real_id(last_item_id)

        feat = self.embedding_dict[real_id][:self.frame_no]
        sample_items[mask_len_head + tokens_Len] = feat
...
class SVFrameEvalDataset(Dataset):
    def __init__(self, item_ids, item_id_to_keys, embedding_dict, frame_no):
        self.item_ids = item_ids  # e.g., np.arange(item_num+1)
        self.item_id_to_keys = item_id_to_keys
        self.embedding_dict = embedding_dict
        self.frame_no = frame_no
        
        example_key = next(iter(embedding_dict))
        self.embed_dim = embedding_dict[example_key].shape[1]
    ...
    def __getitem__(self, idx):
        ...
        if item_id == 0:
            # padding item
            return torch.zeros(self.frame_no, self.embed_dim)

        real_id = self.item_id_to_keys[item_id]

        feat = self.embedding_dict[real_id][:self.frame_no]
        return feat # (frame_no, embed_dim)
...
class DINOFrameDataset(Dataset):
    def __init__(self, u2seq, item_num, max_seq_len, item_id_to_keys, embedding_path, frame_no):
        ...
        self.frame_no = frame_no
        example_key = next(iter(self.embedding_dict))
        self.embed_dim = self.embedding_dict[example_key].shape[0]
    ...
    def __getitem__(self, user_id):
        ...
        for i in range(tokens_Len):
            ...
            for f in range(1, self.frame_no + 1):
                feat = self.embedding_dict[(real_id, f)]
                sample_items[mask_len_head + i, f-1] = feat.numpy()
        ...
        for f in range(1, self.frame_no + 1):
            feat = self.embedding_dict[(real_id, f)]
            sample_items[mask_len_head + tokens_Len, f-1] = feat.numpy()
...
class DINOFrameEvalDataset(Dataset):
    def __init__(self, item_ids, item_id_to_keys, embedding_dict, frame_no):
        self.item_ids = item_ids  # e.g., np.arange(item_num+1)
        self.item_id_to_keys = item_id_to_keys
        self.embedding_dict = embedding_dict
        ...
        example_key = next(iter(embedding_dict))
        self.embed_dim = embedding_dict[example_key].shape[0]
    ...
    def __getitem__(self, idx):
        ...
        frames = [self.embedding_dict[(real_id, f)] 
                  for f in range(1, self.frame_no + 1)]
        return torch.stack(frames, dim=0) # (frame_no, embed_dim)
