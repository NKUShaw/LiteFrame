...
def read_videos_from_pairs(pairs_csv_path):
    import pandas as pd
    
    df = pd.read_csv(pairs_csv_path, header=None)
    df.columns = ["user_id", "seq"]
    ...
    # 排序可选
    unique_real_ids = sorted(list(all_real_ids))

    # 构建两张表
    item_name_to_id = {rid: idx+1 for idx, rid in enumerate(unique_real_ids)}
    item_id_to_keys = {idx+1: rid for idx, rid in enumerate(unique_real_ids)}

    print("实际视频数量 =", len(unique_real_ids))
    return item_id_to_keys, item_name_to_id
...
def read_behaviors_SV(before_item_id_to_keys, before_item_name_to_id, Log_file, args):
    behaviors_path = os.path.join(args.root_data_dir, args.dataset, args.behaviors)
    max_seq_len, min_seq_len = args.max_seq_len, args.min_seq_len
    ...
    for user_name, item_seqs in user_seq_dic.items():
        ...
        n_total = len(user_seq)
        n_train = int(n_total * 0.8)
        n_valid = int(n_total * 0.1)
        
        if n_valid == 0 and n_total > 2:
            n_valid = 1
        if (n_total - n_train - n_valid) == 0 and n_total > 1:
             n_train = n_total - 2
             n_valid = 1
        
        train = user_seq[:n_train]
        valid = user_seq[n_train : n_train + n_valid]
        test = user_seq[n_train + n_valid:]
        users_train[user_id] = train
        users_valid[user_id] = valid  # 验证集的预测目标 (Targets)
        users_test[user_id] = test    # 测试集的预测目标 (Targets)
        ...
        users_history_for_valid_and_train = train + valid
        users_history_for_test[user_id] = torch.LongTensor(np.array(users_history_for_valid_and_train))
